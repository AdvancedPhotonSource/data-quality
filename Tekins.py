...
def parse_arguments():
  ...
  parser.add_argument('zmq_host', default='local',
                      help='host providing data over Zmq')
  parser.add_argument('zmq_port', default='5577',
                      help='port to connect to data provider over Zmq')
...
class ZmqImageTransfer:
  def __init__(self, publisher_socket, zmq_host, zmq_port, builder,
               beg_sinogram=0, num_sinograms=0, seq=0):
    self.publisher_socket = publisher_socket
    self.builder = builder
    self.beg_sinogram = beg_sinogram
    ...
    self.socket.connect("tcp://" + zmq_host +":%s" % zmq_port)
  ...
  def receive_zmq_send(self):
      ...
      while not interrupted:
          ...
          if key == "end":
              ...
              self.dims = (msg[data.dim_x], msg[data.dim_y])
              if self.num_sinograms > 0:
                  if (self.beg_sinogram < 0) or (self.beg_sinogram + self.num_sinograms > self.dims[0]):
                      raise Exception("Exceeds the sinogram boundary: {} vs. {}".format(
                          self.beg_sinogram + self.num_sinograms, self.dims[0]))
              ...
              image = np.frombuffer(self.socket.recv(), dtype=dtype).reshape(shape)
              self.push_image_data(image_number, image, theta)
  ...
  def push_image_data(self, image_number, image, theta):
    if self.num_sinograms != 0:
      img = img[self.beg_index: self.end_index]
      img = img.reshape((self.num_sinograms, self.dims[1]))
    else:
...
def main():
  ...
  elif args.daq_mod == 3: # read data from zmq
    with ZmqImageTransfer(publisher_socket=publisher_socket,
                        zmq_host=args.zmq_host, zmq_port=args.zmq_port,
                        builder=builder, beg_sinogram=args.beg_sinogram,
                        num_sinograms=args.num_sinograms, seq=0) as tdet:
      tdet.receive_zmq_send()
